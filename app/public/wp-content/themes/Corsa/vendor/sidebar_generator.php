...
class sidebar_generator {
	...
	public static function admin_print_scripts() {
		...
		$ajax_nonce = wp_create_nonce( "us-sidebar" );
		?>
		<script>
			function add_sidebar(sidebar_name){
				...
				mysack.setVar("security", '<?php echo $ajax_nonce; ?>');
	...
	public static function add_sidebar() {
		check_ajax_referer( 'us-sidebar', 'security' );

		$sidebars = sidebar_generator::get_sidebars();
		$name = sanitize_text_field( str_replace( array( "\n", "\r", "\t" ), '', $_POST['sidebar_name'] ) );
	...
	public static function save_form( $post_id ) {
		if ( ! empty( $_POST['sbg_edit'] ) ) {
			...
			add_post_meta( $post_id, 'sbg_selected_sidebar', sanitize_text_field( $_POST['sidebar_generator'] ) );
			add_post_meta( $post_id, 'sbg_selected_sidebar_replacement', sanitize_text_field( $_POST['sidebar_generator_replacement'] ) );
		}
	}
