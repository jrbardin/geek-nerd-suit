...
function us_theme_activation()
{
	...
	if (is_admin() && $pagenow == 'themes.php' && isset($_GET['activated']))
	{
		...
		update_user_option( $user->ID, 'corsa_cpt_in_menu_set', false, true );
...
function us_include_cpt_to_menu() {
	...
	if ( is_admin() AND $pagenow == 'nav-menus.php' ) {
		$already_set = get_user_option( 'corsa_cpt_in_menu_set' );
		...
		if ( ! $already_set ) {
			...
			if ($hidden_meta_boxes !== false) {
				...
				update_user_option( $user->ID, 'corsa_cpt_in_menu_set', true, true );
			}
		}
	}
}
...
/**
 * Get current Impreza Theme version
