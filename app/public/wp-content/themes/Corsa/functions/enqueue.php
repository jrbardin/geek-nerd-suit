...
function us_enqueue_fonts() {
	...
	foreach ( $prefixes as $prefix ){
		...
		// Fault tolerance for missing font-weights
...
function us_styles()
{
	...
	wp_register_style('us-font-awesome', $template_directory_uri.'/css/font-awesome.css', array(), '4.6.0', 'all');
...
function us_custom_styles()
{
	...
	$styles_file = $styles_dir.'/us_corsa_custom_styles.css';

	if (get_template_directory_uri() !=  get_stylesheet_directory_uri())
	{
		wp_register_style('corsa-style' ,  str_replace( array( 'http:', 'https:' ), '', get_stylesheet_directory_uri() ) . '/style.css', array(), $theme_version, 'all');
		wp_enqueue_style('corsa-style');
	}

	if (file_exists($styles_file))
	{
		wp_register_style('us_custom_css', str_replace( array( 'http:', 'https:' ), '', $wp_upload_dir['baseurl'] ) . '/us_custom_css/us_corsa_custom_styles.css', array(), $theme_version, 'all');
...
function us_jscripts()
{
	...
	wp_register_script('us-supersized', $template_directory_url.'/js/supersized.3.2.7.js', array('jquery'), '', FALSE);
	wp_register_script('us-supersized-shutter', $template_directory_url.'/js/supersized.shutter.js', array('jquery'), '', TRUE);
	wp_enqueue_script('us-supersized');
	wp_enqueue_script('us-supersized-shutter');
